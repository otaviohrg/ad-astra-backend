...
class SimulationPickleRepository(ISimulationRepository):
    ...
    def get(self, simulation_id: str) -> Simulation:
        try:
            ...
            with open(Path(self.storage_dir) / simulation_id, mode="rb") as entry_file:
    ...
    def add(self, simulation: Simulation) -> None:
        with open(Path(self.storage_dir) / simulation.id, mode="wb") as entry_file:
            pickle.dump(simulation, entry_file)

    def remove(self, simulation_id: str) -> None:
        os.remove(Path(self.storage_dir) / simulation_id)
    ...
    def edit(self, content: Dict[str, str]) -> None:
        ...
        with open(Path(self.storage_dir) / simulation_id, mode="wb") as entry_file:
            pickle.dump(simulation, entry_file)

    def get_all(self) -> List[Simulation]:
        simulations: List[Simulation] = []
        for simulation_file_path in Path(self.storage_dir).iterdir():
